...
try:
    ...
    # Scroll down to load more images (optional, adjust as needed)
...
# Find all image tags with specific attributes
img_tags = soup.find_all("img", {"class": lambda x: x and "gallery-asset__thumb" in x})
logging.info(f"Found {len(img_tags)} image tags with 'gallery-asset__thumb' class.")
...
for idx, img in enumerate(img_tags):
    ...
    if asset_id == "82567379":
        logging.info(f"Found target image ID: {asset_id}")
    # Comment out the above condition if you want all images, not just ID 82567379
    ...
    try:
        ...
        logging.info(f"Downloaded: {img_name} (Asset ID: {asset_id})")
    except Exception as e:
